...
RUN python -m grpc_tools.protoc -I. --python_out=. --grpc_python_out=. img_embedding.proto
...
COPY --from=builder --chown=${USER}:${GROUP} ${WORKSPACE}/img_embedding.proto /

# Copy generated .py files to workspace
COPY --from=builder --chown=${USER}:${GROUP} ${WORKSPACE}/*.py ${WORKSPACE}/

# Copy files to workspace
COPY --chown=${USER}:${GROUP} img_embedding_server.py ${WORKSPACE}/img_embedding_server.py
COPY --chown=${USER}:${GROUP} vgg16-397923af.pth ${WORKSPACE}/vgg16-397923af.pth
...
CMD ["python", "img_embedding_server.py"]
